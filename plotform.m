...
a = 1;
for m = 1:5:100 % m < n number of past values to keep
...
%hold off
%loglog(tt,Gexact,'-o','Displayname', 'Analytical solution');
%hold on;
%loglog(tt,Gn,'b-o','Displayname', 'Full model');
%loglog(tt,G0,'k-o','Displayname', 'ODE solution'); 
%loglog(tt,Mnp,'-o','Displayname', 'Reduced model');

err(a)=norm(Gexact-Mnp)/norm(Gexact);
a = a+1;
end
